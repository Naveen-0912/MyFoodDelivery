...
class AllHotelsAdapter(var hotels: List<HotelAccount>, var favoriteHotels: List<Int>, private val context: Context) :
    RecyclerView.Adapter<AllHotelsAdapter.HotelViewHolder>() {
    ...
    private fun likeButtonListener(isLiked: Boolean, imageButton: ImageButton, hotelId: Int) {
        ...
        onLikeClickListener?.onClick(hotelId, isLiked)
    }

    private fun setUpCardDetailsPortrait(binding: HotelCardPortaitBinding, position: Int) {
        val curHotel = hotels[position]
        var isLiked = curHotel.hotelId in favoriteHotels
        binding.ivHotelPicture.setImageResource(curHotel.imageId)
        binding.tvHotelName.text = curHotel.name
        binding.tvHotelRating.text =
            if (curHotel.ratings.averageRatings < 0f) context.resources.getString(R.string.empty_ratings) else curHotel.ratings.averageRatings.toString()
        ...
        binding.btnFavorite.setOnClickListener {
            isLiked = !isLiked
            likeButtonListener(isLiked, binding.btnFavorite, curHotel.hotelId)
        }
        if (curHotel.isVeg)
    ...
    override fun onBindViewHolder(holder: HotelViewHolder, position: Int) {
        with(holder) {
            if (binding is HotelCardPortaitBinding)
                setUpCardDetailsPortrait(binding, position)
            else if (binding is HotelCardLandscapeBinding)
                setUpCardDetailsLandscape(binding, position)
        }
    }
}
